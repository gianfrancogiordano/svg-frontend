...
export class UsuarioService {
  ...
  getUsuarios(pagina: number, porPagina: number, filtro: string, tipoUsuario: string = '') {
    return this.http.get(`${base_url}/usuarios?pagina=${pagina}&porPagina=${porPagina}&filtro=${filtro}&tipousuario=${tipoUsuario}`, this.headers )
      .pipe(
  ...
  validarToken(): Observable<boolean> {
    return this.http.get(`${ base_url }/login/renew`, this.headers ).pipe(
      map((resp: any) => {
        ...
        const negocio = resp.usuarioBD.negocio;
        const role = resp.usuarioBD.role;
        
        this.usuario = new Usuario(usuario.nombre, usuario.user, usuario.email, '', usuario.firma, role, negocio._id, negocio, usuario._id);
        this.guardarStorage( resp.token, resp.menu );
